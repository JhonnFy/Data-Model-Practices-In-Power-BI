...
SELECT SUM(a.total_price) / COUNT(DISTINCT a.order_id) AS Avg_Order_Value FROM pizza_sales a
GO

--3. Total Pizzas Sold
SELECT SUM(a.total_price) AS Total_Revenue FROM pizza_sales a
GO

--4. Total Orders
SELECT SUM(a.quantity) AS Total_Pizza_Sold FROM pizza_sales a
GO

--5. Average Pizzas Per Order
SELECT COUNT(DISTINCT a.order_id) AS Total_Orders FROM pizza_sales a
GO
...
--17. L. Borrom 5 Pizzas by Total Orders
SELECT Top 5 pizza_name, COUNT(DISTINCT order_id) AS Total_Orders
FROM pizza_sales
GROUP BY pizza_name
